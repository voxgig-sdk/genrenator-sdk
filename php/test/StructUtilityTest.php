...
// Genrenator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = GenrenatorOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(GenrenatorOmni::structfix($now, Struct::undef()))
                        !== json_encode(GenrenatorOmni::structfix($val, Struct::undef()))) {
