...
class FtCtrl extends GenrenatorControl
{
...
class FtClient
{
    ...
    private GenrenatorUtility $utility;

    public function __construct(GenrenatorUtility $utility, array $options)
    {
    ...
    public function get_utility(): GenrenatorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GenrenatorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GenrenatorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GenrenatorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GenrenatorConfig::make_feature($name);
    ...
    private static function buildUrl(GenrenatorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GenrenatorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GenrenatorResponse($fetched);
        }
    ...
    private function fail(GenrenatorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GenrenatorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
