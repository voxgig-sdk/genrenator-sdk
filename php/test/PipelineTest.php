...
{
    private static function utility(): GenrenatorUtility
    {
        return new GenrenatorUtility();
    }
    ...
    private static function ctx(array $over = []): GenrenatorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GenrenatorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GenrenatorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GenrenatorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GenrenatorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GenrenatorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GenrenatorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GenrenatorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GenrenatorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GenrenatorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GenrenatorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GenrenatorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GenrenatorResponse([]),
            'result' => new GenrenatorResult([]),
        ]);
        [, $err] = GenrenatorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GenrenatorSpec([]),
            'response' => null,
            'result' => new GenrenatorResult([]),
        ]);
        [, $err] = GenrenatorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GenrenatorSpec(['step' => 's']),
            'response' => new GenrenatorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GenrenatorResult([]),
        ]);
        [, $err] = GenrenatorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GenrenatorSpec(['step' => 's']),
            'response' => new GenrenatorResponse(self::resp(200, ['v' => 1])),
            'result' => new GenrenatorResult([]),
        ]);
        [, $err] = GenrenatorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GenrenatorSpec(['step' => 's']),
            'response' => new GenrenatorResponse(self::resp(200, ['v' => 2])),
            'result' => new GenrenatorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GenrenatorMakeResponse::call($ctx);
    ...
    {
        $preset = new GenrenatorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GenrenatorSpec([]),
            'response' => new GenrenatorResponse([]),
            'result' => new GenrenatorResult([]),
        ...
        [$got, $err] = GenrenatorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GenrenatorResult([])]);
        [, $err] = GenrenatorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GenrenatorSpec([]), 'result' => null]);
        [, $err] = GenrenatorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GenrenatorSpec(['step' => 's']),
            'result' => new GenrenatorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GenrenatorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GenrenatorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GenrenatorSpec(['step' => 's']),
            'result' => new GenrenatorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GenrenatorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GenrenatorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GenrenatorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GenrenatorSpec([]),
            'result' => new GenrenatorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GenrenatorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GenrenatorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GenrenatorError('boom', 'boom');
        $utility->fetcher = function (GenrenatorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GenrenatorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GenrenatorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GenrenatorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GenrenatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GenrenatorResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GenrenatorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GenrenatorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GenrenatorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GenrenatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GenrenatorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GenrenatorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GenrenatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GenrenatorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GenrenatorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GenrenatorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GenrenatorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GenrenatorResult(['ok' => false])]);
        $this->expectException(GenrenatorError::class);
        GenrenatorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GenrenatorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GenrenatorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GenrenatorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GenrenatorBaseFeature();
        $b = new GenrenatorBaseFeature();
        GenrenatorFeatureAdd::call($ctx, $a);
        GenrenatorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GenrenatorBaseFeature
    {
        $f = new GenrenatorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GenrenatorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GenrenatorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GenrenatorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GenrenatorPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GenrenatorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GenrenatorResponse(['headers' => null]),
            'result' => new GenrenatorResult([]),
        ]);
        GenrenatorResultHeaders::call($ctx);
