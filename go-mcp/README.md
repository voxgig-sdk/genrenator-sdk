...
[MCP](https://modelcontextprotocol.io) server exposing the Genrenator SDK as
two agent tools — `genrenator_list` and `genrenator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/genrenator-mcp)
make build

# 2. Provide credentials via the environment
export GENRENATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user genrenator \
  -- /absolute/path/to/genrenator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./genrenator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// genrenator_list: first page of records
{ "entity": "genre" }
{ "entity": "genre", "query": { } }

// genrenator_load: one record by id
{ "entity": "genre", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/genrenator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user genrenator \
     -- "$PWD"/dist/*/genrenator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `genrenator_list` and `genrenator_load` tools now appear
   in new sessions. Ask the agent to *"list genre using genrenator"*
   and it calls `genrenator_list` with `{"entity":"genre"}`.
...
```sh
export GENRENATOR_APIKEY=sk_live_xxx            # API key
export GENRENATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `genrenator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `genrenator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GENRENATOR_APIKEY` | API key sent with every request. |
| `GENRENATOR_BASE` | Optional override of the API base URL. |
...
```sh
./genrenator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"genrenator_load","arguments":{"entity":"genre","query":{"id":1}}}}'
```
