...
MCP server exposing the Genrenator SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `genrenator_list` | `entity`, optional `query` map | First page of records as JSON |
| `genrenator_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `genrenator_list` and
`genrenator_load` tools become available in new sessions.
