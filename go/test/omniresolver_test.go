...
import (
	...
	omni "github.com/voxgig-sdk/genrenator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GenrenatorError); is {
				out := map[string]any{
					"name":    "GenrenatorError",
...
func omniCtx(arg any, client *sdk.GenrenatorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GenrenatorSDK); is {
			client = live
