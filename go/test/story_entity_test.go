...
func TestStoryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GENRENATOR_TEST_STORY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if storyRef01DataDt0Loaded == nil {
...
func storyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GENRENATOR_TEST_STORY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GENRENATOR_TEST_STORY_ENTID"])
	...
	if env["GENRENATOR_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewGenrenatorSDK(core.ToMapAny(mergedOpts))
	}
