...
import (
	...
	vs "github.com/voxgig-sdk/genrenator-sdk/go/utility/struct"
)

type GenrenatorSDK struct {
	Mode     string
...
func (sdk *GenrenatorSDK) OptionsMap() map[string]any {
...
func (sdk *GenrenatorSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *GenrenatorSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *GenrenatorSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GenrenatorSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *GenrenatorSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *GenrenatorSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("GenrenatorSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *GenrenatorSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GenrenatorSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("GenrenatorSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
