...
//     (\`new GenrenatorSDK(...)\` / \`GenrenatorSDK.test(...)\` become
//     \`GenrenatorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"genre":{"test01":{"id":"test01"}},"story":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GenrenatorSDK(...)\` and any
// \`GenrenatorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GenrenatorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GenrenatorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
