...
class StoryEntity
  ...
  def data_set(args)
    if args
      @_data = GenrenatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = GenrenatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Story, Hash] the loaded Story; raises GenrenatorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GenrenatorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
