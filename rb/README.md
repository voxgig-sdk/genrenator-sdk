...
The Ruby SDK for the Genrenator API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Genre` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = GenrenatorSDK.test(testopts, sdkopts)
```
...
raise a `GenrenatorError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Genrenator_sdk.rb       -- Main SDK module
...
The main module (`Genrenator_sdk`) exports the SDK class
