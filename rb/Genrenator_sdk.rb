...
require_relative 'Genrenator_types'
...
class GenrenatorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GenrenatorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GenrenatorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GenrenatorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GenrenatorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GenrenatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GenrenatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GenrenatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => GenrenatorError.new(
        "#{op}_allow",
        "GenrenatorSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GenrenatorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GenrenatorError.new(
        "graphql_error", "GenrenatorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
