...
class GenrenatorRetryFeature extends GenrenatorBaseFeature
{
    ...
    public function init(GenrenatorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GenrenatorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GenrenatorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GenrenatorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
