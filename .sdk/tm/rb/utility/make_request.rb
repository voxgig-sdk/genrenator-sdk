...
module GenrenatorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GenrenatorResponse.new({})
    ...
    if fetch_err
      ...
      response = GenrenatorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GenrenatorResponse.new(fetched)
    else
