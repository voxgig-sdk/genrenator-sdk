...
from projectname_sdk.core.control import GenrenatorControl
...
class GenrenatorContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, GenrenatorSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, GenrenatorResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, GenrenatorResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return GenrenatorError(code, msg, self)
