...
The Python SDK for the Genrenator API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Genre(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Genrenator(None).load(
    {"id": "test01"}, None
)
