# Genrenator SDK test runner

from __future__ import annotations
import os
import json

from genrenator_sdk.utility.voxgig_struct import voxgig_struct as vs


class GenrenatorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GenrenatorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GenrenatorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GenrenatorTestRunner.getenv("GENRENATOR_TEST_LIVE")
        override = GenrenatorTestRunner.getenv("GENRENATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GenrenatorTestRunner.getenv(key)
        ...
        explain = GenrenatorTestRunner.getenv("GENRENATOR_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GENRENATOR_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GenrenatorTestRunner._test_control is not None:
            return GenrenatorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GenrenatorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GenrenatorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GenrenatorTestRunner.load_test_control()
...
def load_env_local():
    GenrenatorTestRunner.load_env_local()


def env_override(m):
    return GenrenatorTestRunner.env_override(m)


def entity_data(v):
    return GenrenatorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GenrenatorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GenrenatorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GenrenatorTestRunner.load_test_control()


def live_client_options():
    return GenrenatorTestRunner.live_client_options()


def live_delay_ms():
    return GenrenatorTestRunner.live_delay_ms()
