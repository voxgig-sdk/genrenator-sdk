...
The Lua SDK for the Genrenator API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Genre(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Genrenator(nil):load(
  { id = "test01" }, nil
)
