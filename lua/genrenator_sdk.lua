...
local GenrenatorSDK = {}
GenrenatorSDK.__index = GenrenatorSDK
...
GenrenatorSDK._make_feature = _make_feature


function GenrenatorSDK.new(options)
  local self = setmetatable({}, GenrenatorSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function GenrenatorSDK:options_map()
...
function GenrenatorSDK:get_utility()
  return Utility.copy(self._utility)
end


function GenrenatorSDK:get_root_ctx()
  return self._rootctx
end


function GenrenatorSDK:prepare(fetchargs)
...
function GenrenatorSDK:direct(fetchargs)
...
-- by passing it.
function GenrenatorSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function GenrenatorSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "GenrenatorSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function GenrenatorSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = GenrenatorSDK.new(sdkopts)
  sdk.mode = "test"
