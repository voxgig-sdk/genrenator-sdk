...
boru-driven command-line client **and** interactive REPL for the Genrenator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/genrenator-cli)
make build
...
./genrenator-cli load 1 genre            # {id:1} shorthand
./genrenator-cli load '{id:1}' genre       # explicit match map

# 5. Override the API base URL for a single call
GENRENATOR_BASE=https://api.example.com ./genrenator-cli load 1 genre

# 6. No arguments -> interactive REPL
./genrenator-cli
genrenator> load 1 genre
genrenator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/genrenator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/genrenator-cli load 1 genre
   ```
...
```sh
./genrenator-cli load 1 genre          # scalar shorthand for {id:1}
./genrenator-cli load '{id:1}' genre     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `genrenator>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `genre`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/genrenator-cli`. |
...
(`./genrenator-cli <expr>`) and interactively (the REPL), and expressions compose the
