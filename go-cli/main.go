// Command Genrenator-cli is a CLI + REPL for the Genrenator SDK,
...
const prompt = "genrenator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "genre story"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GENRENATOR_APIKEY (api key), GENRENATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GENRENATOR_APIKEY carries the API key and
	// GENRENATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GENRENATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GENRENATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGenrenatorSDK(opts)
